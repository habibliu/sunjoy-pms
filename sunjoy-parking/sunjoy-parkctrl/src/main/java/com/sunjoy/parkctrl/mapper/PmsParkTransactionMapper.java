...
public interface PmsParkTransactionMapper {
    @Insert("INSERT INTO pms_park_transaction (entry_ref_id,tenant_id, opu_id, park_id, park_name, vehicle_id, " +
            "license_plate, entry_service_id, entry_lane_id, entry_lane_name, entry_device_id, " +
            "entry_time, entry_rel_time, entry_rel_mode, lot_no,exit_ref_id,exit_service_id,  exit_lane_id, exit_lane_name, " +
            "exit_device_id, exit_time, exit_rel_time, exit_rel_mode, parking_duration, status, " +
            "del_flag, create_by, create_time, update_by, update_time, remark) " +
            "VALUES (#{entryRefId},#{tenantId}, #{opuId}, #{parkId}, #{parkName}, #{vehicleId}, #{licensePlate}, " +
            "#{entryServiceId}, #{entryLaneId}, #{entryLaneName}, #{entryDeviceId}, #{entryTime}, " +
            "#{entryRelTime}, #{entryRelMode}, #{lotNo},#{exitRefId}, #{exitServiceId},#{exitLaneId}, #{exitLaneName}, " +
