    ...
    <div class="app-container">
      <el-row :gutter="20">
        ...
        <el-table v-loading="loading" :data="data" @selection-change="handleSelectionChange">
          ...
          <el-table-column label="通道编号" align="center" key="laneId" prop="laneId" v-if="columns[0].visible" width="100"/>
          <el-table-column label="通道名称" align="left" key="laneName" prop="laneName" v-if="columns[1].visible" :show-overflow-tooltip="true" />
          <el-table-column label="进出方向" align="center" key="direction" prop="direction" v-if="columns[2].visible" :show-overflow-tooltip="true" />
          <el-table-column label="连通外围" align="center" key="isOuter" prop="isOuter" v-if="columns[3].visible" :show-overflow-tooltip="true"/>
          <el-table-column label="收费放行" align="center" key="rap" prop="rap" v-if="columns[4].visible" :show-overflow-tooltip="true"/>
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="通道名称" prop="laneName">
              <el-input v-model="form.laneName" placeholder="请输入用户昵称" maxlength="30" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="24">
            <el-form-item label="进出方向" prop="direction">
              <el-select v-model="form.direction" placeholder="请选择车辆出入场方向">
                <el-option
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="是否外围" prop="isOuter">
              <el-select v-model="form.isOuter" placeholder="请选择是否与外界连通">
                <el-option
      ...
      return {
        ...
        rules: {
          laneName: [
            ...
            { min: 2, max: 20, message: '通道名称长度必须介于 2 和 20 之间', trigger: 'blur' }
          ],
          direction: [
    ...
    methods: {
      ...
      handleDelete(row){
        ...
        this.$modal.confirm('是否确认删除当前行的通道？').then(function() {
          that.data=that.data.filter(lane => lane.laneName !== row.laneName);
     ...
     addLane () {
        this.$refs["form"].validate(valid => {
          if (valid) {
            ...
            this.data.push(Object.assign({}, this.form));
